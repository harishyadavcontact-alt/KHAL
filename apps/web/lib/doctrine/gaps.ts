...
export function unresolvedDoctrineGapByDomain(sources: VolatilitySourceDto[] = [], responseLogic: WarGameDoctrineChain[] = []): Map<string, DoctrineGapCode> {
  ...
  for (const source of sources) {
    for (const profile of source.mapProfiles ?? []) {
      ...
      if (!craftId) continue;
      const gap = doctrineGapForCraft(craftId, responseLogic);
      if (gap) out.set(profile.domainId, gap);
    }
  }
  return out;
}
